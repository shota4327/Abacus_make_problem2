/**
 * @file multiplicationValidator.ts
 * @description 掛け算問題(10問)の各項（被乗数・乗数）および全体の数字出現頻度、有効桁数、連続桁マトリクス等の統計情報を計算する集計・検証ユーティリティです。
 */

// 共通バリデーションユーティリティのインポート
import { calculateFrequency, calculateTotalFrequency, calculateRowDigitCounts } from './validatorUtils';
import { MultiplicationProblemState, MultiplicationStats } from '../types';
 ...
 * 
 * @param problems - 10問分の掛け算問題オブジェクト配列
 * @returns 集計結果オブジェクト（各辺および全体の頻度、桁数、目標差分、連続文字等）
 */
export const calculateMultiplicationStats = (problems: MultiplicationProblemState[]): MultiplicationStats => {
    // 1. 左辺（被乗数）の統計集計
    ...
    const frequencyAll = frequencyLeft.map((leftRowCounts, index) => {
        const rightRow = frequencyRight[index];
        return leftRowCounts.map((count, digit) => {
            const rightVal = rightRow ? (rightRow[digit] ?? 0) : 0;
            return count + rightVal;
        });
    });
    const totalFrequencyAll = calculateTotalFrequency(frequencyAll);
    const rowDigitCountsAll = rowDigitCountsLeft.map((leftCount, index) => leftCount + (rowDigitCountsRight[index] ?? 0));
    ...
    const consecutive: number[][] = Array(10).fill(null).map(() => Array(10).fill(0)); // [d1][d2] マトリクス

    problems.forEach(p => {
        // 左辺の連続文字チェック（先頭のゼロはスキップ）
        let foundNonZeroLeft = false;
        let lastValidLeft: number | null = null;
        for (let i = 0; i < p.left.length; i++) {
            ...
            if (current !== null && current !== undefined && (current as unknown) !== '') {
                ...
                if (lastValidLeft !== null) {
                    const cRow = consecutive[lastValidLeft];
                    if (cRow) {
                        const curCount = cRow[num];
        ...
        let lastValidRight: number | null = null;
        for (let i = 0; i < p.right.length; i++) {
            ...
            if (current !== null && current !== undefined && (current as unknown) !== '') {
                ...
                if (lastValidRight !== null) {
                    const cRow = consecutive[lastValidRight];
                    if (cRow) {
                        const curCount = cRow[num];
                        if (curCount !== undefined) {
                            cRow[num] = curCount + 1;
