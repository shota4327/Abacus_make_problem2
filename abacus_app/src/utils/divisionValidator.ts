...
import { DivisionProblemState, DivisionStats } from '../types';
 ...
 * 
 * @param problems - 10問分の割り算問題オブジェクト配列
 * @returns 集計結果オブジェクト（割る数・商・全体の出現頻度、桁数、目標差分、連続文字等）
 */
export const calculateDivisionStats = (problems: DivisionProblemState[]): DivisionStats => {
    // 1. 全体（割る数 + 答え）の集計
    ...
    const countConsecutivePairs = (row: (number | null)[]) => {
        ...
        for (let i = 0; i < row.length; i++) {
            ...
            if (current !== null && current !== undefined && (current as unknown) !== '') {
                ...
                foundNonZero = true;
                if (lastValid !== null) {
                    const cRow = consecutive[lastValid];
                    if (cRow) {
