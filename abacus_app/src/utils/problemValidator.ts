/**
 * @file problemValidator.ts
 * @description 見取り算問題（20行×13列の盤面）が作問条件（桁数、特定の数字出現率、包み・挟み・連続桁、初口/最終口/答えの先頭・末尾桁など）を満たしているか検証し、各種統計情報を算出する検証・集計モジュールです。
 */

import { COL_COUNT } from '../constants/initialState';
import { Grid, ProblemConditions, ProblemStats } from '../types';

/**
 * 盤面の各種統計情報および条件判定の成否を計算します。
 *
 * @param grid - 問題の盤面データ（20x13セル）
 * @param isMinusRows - 各行がマイナス（引き算）かどうかを表すフラグ配列
 * @param rowCount - 有効な行数（口数）
 * @param targetTotalDigits - 目標とする全体合計桁数（各数字平均出現数の計算等で使用）
 * @param conditions - 作問制約条件
 * @returns 計算結果の統計データと条件検証フラグ
 */
export const calculateProblemStats = (
    grid: Grid,
    isMinusRows: boolean[],
    rowCount: number,
    ...
    const frequency: number[][] = [];          // 各行ごとの数字(0-9)出現頻度配列 [row][digit]
    const totalFrequency: number[] = Array(10).fill(0);  // 全行を通じた数字(0-9)の合計出現頻度
    const consecutive: number[][] = Array(10).fill(null).map(() => Array(10).fill(0)); // 連続数字の出現回数マトリクス [d1][d2]
    const rowDigitCounts: number[] = [];                 // 各行の有効桁数配列
    ...
    for (let rowIndex = 0; rowIndex < rowCount; rowIndex++) {
        ...
        const rowFreq: number[] = Array(10).fill(0);
        ...
        row.forEach((digit, colIndex) => {
            ...
            rowFreq[currentDigit] = (rowFreq[currentDigit] ?? 0) + 1;
            const curTotal = totalFrequency[currentDigit];
            if (curTotal !== undefined) {
                totalFrequency[currentDigit] = curTotal + 1;
            }
            rowDigitCount++;
            ...
            if (colIndex < COL_COUNT - 1) {
                ...
                const nextDigit = nextCell === null || nextCell === undefined ? 0 : nextCell;
                const rowConsecutive = consecutive[currentDigit];
                if (rowConsecutive) {
                    const count = rowConsecutive[nextDigit];
                    if (count !== undefined) {
                        rowConsecutive[nextDigit] = count + 1;
                    }
                }
            }
        });
        ...
        const isMinus = isMinusRows[rowIndex] ?? false;
        const rowValue = (parseInt(rowValueString, 10) || 0) * (isMinus ? -1 : 1);
        totalSum += rowValue;
    }
    ...
    const frequencyDiffs: number[] = Array(10).fill(0).map((_, digit) => {
        ...
        const actualFreq = totalFrequency[digit] ?? 0;
        return actualFreq - baseline;
    });
    ...
    if (!isConsecutiveUsed) {
        ...
        const consecRow = consecutive[target];
        if (!isNaN(target) && consecRow && (consecRow[target] ?? 0) > 0) {
     ...
     * @param target - 指定目標値
     * @param actual - 実際の値
     * @returns 一致していればtrue（未指定は常にtrue）
     */
    const checkMatch = (target: number | string | null, actual: number | string | null): boolean => {
     ...
     * @returns 最上位桁と最下位桁
     */
    const getMostAndLeastSignificantDigit = (rowArr: (number | null)[] | undefined) => {
        let msd: number | null = null; // 最上位桁 (Most Significant Digit)
        let lsd: number | null = null; // 最下位桁 (Least Significant Digit)
        if (rowArr) {
    ...
    const ansMsd = sumString[0] ?? null;
    const ansLsd = sumString[sumString.length - 1] ?? null;
