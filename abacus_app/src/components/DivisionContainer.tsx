...
import { DivisionContainerProps } from '../types';
 ...
 * 
 * @returns {React.ReactElement} 割り算全体UI画面
 */
const DivisionContainer: React.FC<DivisionContainerProps> = () => {
    const {
        problems,
        updateDigit,
        toggleDecimal,
    ...
    const handleUpdateDivisor = (rowIndex: number, length: number): void => {
        regenerateRow(rowIndex, 'divisor', length);
    };

    /** 商（answer）の桁数変更・再生成ハンドラー */
    const handleUpdateAnswer = (rowIndex: number, length: number): void => {
