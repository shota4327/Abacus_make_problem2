/**
 * @file ConsecutiveCounter.tsx
 * @description 2桁の連続する数字ペア（1文字目 -> 2文字目の遷移）の出現頻度マトリクス（10x10のグリッド表）を表示し、連続桁や過多な出現（3回以上）を警告描画するコンポーネントです。
 */

import React from 'react';
import './Stats.css';
import { ConsecutiveCounterProps } from '../types';

/**
 * 連続文字（ペア数値遷移）出現マトリクス表示コンポーネント
 * 
 * @param props - コンポーネントProps
 * @param props.consecutive - 10x10の連続数値出現頻度マトリクス [d1][d2]
 * @returns 連続文字マトリクスUI
 */
const ConsecutiveCounter: React.FC<ConsecutiveCounterProps> = ({ consecutive }) => {
