/**
 * @file DivisionFrequencyCounter.tsx
 * @description 割り算問題の数字別(0-9)出現回数および桁数分布テーブルを表示するコンポーネントです。
 */

import React, { useState, useMemo } from 'react';
import './Stats.css';
import { DivisionFrequencyCounterProps } from '../types';

/**
 * 割り算用の数字出現回数・桁数集計テーブルコンポーネント
 * 
 * @param props - コンポーネントProps
 * @param props.frequency - 各問題・数字ごとの出現頻度
 * @param props.totalFrequency - 全問題の数字別合計頻度
 * @param props.frequencyDiffs - 理想平均値との差分配列
 * @param props.rowDigitCounts - 各問題の桁数
 * @param props.totalRowDigits - 総桁数
 * @param props.targetTotalDigits - 目標総桁数
 * @param props.updateRowDigitCount - 桁数変更・再生成関数
 * @param props.title - コンポーネントタイトル
 * @returns 出現頻度集計テーブル
 */
const DivisionFrequencyCounter: React.FC<DivisionFrequencyCounterProps> = ({
    frequency,
    totalFrequency,
    frequencyDiffs,
    rowDigitCounts,
    ...
    const [activeSelector, setActiveSelector] = useState<{ rowIndex: number } | null>(null);

    const lengths = useMemo<number[]>(() => {
        const min = Math.min(minDigit || 5, maxDigit || 12);
        const max = Math.max(minDigit || 5, maxDigit || 12);
        return Array.from({ length: max - min + 1 }, (_, i) => min + i);
    }, [minDigit, maxDigit]);

    const handleLengthSelect = (rowIndex: number, length: number) => {
                                                                ...
                                                                <button
                                                                    ...
                                                                    onClick={() => {
                                                                        ...
                                                                        if (randomLen !== undefined) {
                                                                            handleLengthSelect(rowIndex, randomLen);
                                                                        }
                                                                    }}
                                                                >
                                                                    R
