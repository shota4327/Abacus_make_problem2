/**
 * @file FrequencyCounter.tsx
 * @description 見取り算・掛け算問題における行別・数字別(0-9)の出現頻度、行ごとの有効桁数、過多警告、および合計差分を表示する統計集計コンポーネントです。
 */

import React, { useState, useMemo } from 'react';
import './Stats.css';
import { FrequencyCounterProps } from '../types';

/**
 * 数字出現頻度および桁数集計パネルコンポーネント
 * 
 * @param props - コンポーネントProps
 * @param props.frequency - 各行ごとの0-9出現回数配列 [row][digit]
 * @param props.totalFrequency - 全行の0-9合計出現回数配列
 * @param props.frequencyDiffs - 理想平均値に対する各数字の過不足差分配列
 * @param props.rowDigitCounts - 各行の有効桁数配列
 * @param props.totalRowDigits - 問題全体の合計有効桁数
 * @param props.targetTotalDigits - 目標とする全体合計桁数
 * @param props.updateRowDigitCount - 行桁数変更・ランダム生成ハンドラー
 * @returns 出現回数統計パネルUI
 */
const FrequencyCounter: React.FC<FrequencyCounterProps> = ({
    frequency,
    totalFrequency,
    frequencyDiffs,
    rowDigitCounts,
